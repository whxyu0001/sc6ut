...
const owner = ["1033580960882626630", "870673139368091699"]
...
m = ch.rtcRegion;
 ...
 .setLabel("Invite")
 .setStyle("LINK")
  .setURL(`https://discord.com/oauth2/authorize?client_id=${client.user.id}&perm    issions=8&scope=applications.commands%20bot`), 
       new MessageButton()
   .setLabel("Support")
 .setStyle("LINK")
 ...
 .setStyle("LINK")  
   .setURL(`https://top.gg/bot/${client.user.id}/vote`), 
      ...
      const embed = new MessageEmbed()
        ...
        .setFooter(`discord.gg/sc6ut`, message.guild.iconURL({dynamic: true}))
  ...
  np = [];
      ...
      if(command.voteonly){
        ...
        if(!vote){
          ...
          .setDescription(`<:s_warn:1093789533734195230> | **This Is A Vote Required Command So Vote Us Now By [Clicking Here](https://top.gg/bot/${client.user.id}/vote)**`)
          .setColor(client.embedColor)
   ...
   if (!channel.permissionsFor(message.guild.me)?.has(Permissions.FLAGS.EMBED_LINKS) && client.user.id !== userId) {
    ...
    if (command.owner) {
     if (!owner.includes(message.author.id)) {
        ...
        embed.setDescription("<:s_cross:1093541595334324326> | Only My Developers Can Use This Command");
        return message.channel.send({embeds: [embed]});
    }
    }
